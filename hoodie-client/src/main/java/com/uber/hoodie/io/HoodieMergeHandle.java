...
public class HoodieMergeHandle<T extends HoodieRecordPayload> extends HoodieIOHandle<T> {
  ...
  public HoodieMergeHandle(HoodieWriteConfig config, String commitTime, HoodieTable<T> hoodieTable,
      Map<String, HoodieRecord<T>> keyToNewRecords, String fileId, Optional<HoodieDataFile> oldDataFileToBeMerged) {
    super(config, commitTime, hoodieTable);
    this.fileSystemView = hoodieTable.getROFileSystemView();
    this.keyToNewRecords = keyToNewRecords;
    if (oldDataFileToBeMerged.isPresent()) {
      init(fileId, FSUtils.getPartitionPathFromFilePath(oldDataFileToBeMerged.get().getFileStatus().getPath()),
          oldDataFileToBeMerged);
    } else {
      init(fileId, keyToNewRecords.get(keyToNewRecords.keySet().stream().findFirst().get())
          .getPartitionPath(), oldDataFileToBeMerged);
    }
  }
   ...
   */
  private void init(String fileId, String partitionPath, Optional<HoodieDataFile> oldDataFileToBeMerged) {
    ...
    try {
      //TODO: oldDataFileToBeMerged must be optional. Will be fixed by Nishith's changes to support insert to log-files
      String latestValidFilePath = oldDataFileToBeMerged.get().getFileName();
