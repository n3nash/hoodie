...
public class ExternalSpillableMap<T,R> implements Map<T,R> {
  ...
  // Schema used to de-serialize and read the records written to disk
  final private Schema schema;
  // An estimate of the size of each payload written to this map
  private volatile int estimatedPayloadSize = 0;
  // TODO (na) : a dynamic sizing factor to ensure we have space for diskBasedMap stuff in memory
   ...
   * @return
   */
  public int getDiskBasedMapSize() {
  ...
  public R put(T key, R value) {
    if (this.currentInMemoryMapSize < maxInMemorySizeInBytes || inMemoryMap.containsKey(key)) {
      ...
      try {
        // TODO (na) : how to track bytes written, use KyroSerializer to serialize the whole payload
        // Naive approach for now
        if(estimatedPayloadSize == 0) {
          this.estimatedPayloadSize = SpillableMapUtils.computePayloadSize(value, this.schema);
        }
      } catch(IOException e) {
        throw new HoodieIOException("unable to estimate size of payload", e);
      }
      currentInMemoryMapSize += key.toString().getBytes().length + this.estimatedPayloadSize;
    } else {
